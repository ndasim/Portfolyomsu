Magnetic = new(function() {
    ...
    function P() {
        ...
        for (u = f.length; h < u; h++) {
			...
			d.arc(b.position.x, b.position.y, 10, 0, Math.PI * 2, true);
            d.fill();
            //d.beginPath();
            //d.fillStyle = k[g].fadeFill;
    ...
    var r = navigator.userAgent.toLowerCase().indexOf("android") != -1 || navigator.userAgent.toLowerCase().indexOf("iphone") != -1 || navigator.userAgent.toLowerCase().indexOf("ipad") != -1,
        ...
        g = 0,
        k = [{
            glowA: "rgba(233,143,154,0)",
            glowB: "rgba(255,255,255,1)",
            particleFill: "rgba(0,0,0,0.5)",
            fadeFill: "rgba(0,0,0,0.5)",
            useFade: false
        },{
            glowA: "rgba(233,143,154,0)",
            glowB: "rgba(0,143,154,0.1)",
            particleFill: "#000000",
    ...
    this.init = function() {
        ...
        if (e && e.getContext) {
            ...
            for (var a = 0; a < 5; a++){ 
				z({
					x: (i - 300) * 0.5 + Math.random() * 500,
					y: (j - 300) * 0.5 + Math.random() * 500
				});
			}
            C();
...
function Particle() {
    ...
    this.angle = 0;
    this.speed = 0.01 + this.size / 4 * 0.08;
    this.force = 1 - Math.random() * 0.1;
    this.color = "#ffffff";
    this.orbit = 1;
    this.magnet = null
}
